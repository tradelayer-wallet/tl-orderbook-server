...
export class OrderbookManager {
  // PUBLIC: other modules (socket manager) will subscribe here
  ...
  constructor () {
    ...
    // if you expect many sockets, avoid EventEmitter warnings
  ...
  removeOrder(uuid: string, socket_id: string): IResult {
    try {
      ...
      // NEW: emit a close-order event on the manager bus for socket layer to forward
      try {
        this.bus.emit('close-order', {
          orderUuid: uuid,
          marketKey: orderbook.orderbookName, // useful for per-market filtering
          socketId: socket_id                 // optional, if callers want it
        });
      } catch {}
